...
SM64_LIB_FN void sm64_set_mario_action_timer(int32_t marioId, uint32_t actionTimer)
{
    if( marioId >= s_mario_instance_pool.size || s_mario_instance_pool.objects[marioId] == NULL )
    {
        DEBUG_PRINT("Tried to use non-existant Mario with ID: %d", marioId);
    ...
    gMarioState->actionTimer = actionTimer;
}
...
SM64_LIB_FN void sm64_set_mario_can_recover(int32_t marioId, uint8_t canRecover)
{
    if( marioId >= s_mario_instance_pool.size || s_mario_instance_pool.objects[marioId] == NULL )
    {
        DEBUG_PRINT("Tried to use non-existant Mario with ID: %d", marioId);
    ...
    gMarioState->canRecover = canRecover;
}
